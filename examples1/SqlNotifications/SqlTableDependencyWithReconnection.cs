      ...
      using (var sqlConnection = new SqlConnection(_connectionString))
      {
        ...
        using (var transaction = sqlConnection.BeginTransaction())
        {
          ...
          var sendDeletedConversationStatements = this.PrepareSendConversation(ChangeType.Delete, interestedColumns);
                    var columns = this.PrepareColumnListForSelectFromTableVariable(tableColumns);
                   var updatechange= this.PrepareInsertIntoTableVariableForUpdateChange(interestedColumns, columnsForUpdateOf).Replace("WHERE 1", string.Empty);
          sqlCommand.CommandText = string.Format(
             ...
             //this.CreateWhereCondition(true),
             string.Empty,

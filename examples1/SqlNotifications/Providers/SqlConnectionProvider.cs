...
public class SqlConnectionProvider : ISqlConnectionProvider
    {
        ...
        public Guid GetConversationHandler(string connectionString, string farService)
        {
            ...
            {
                sqlConnection.Open();
                string command =$"SELECT conversation_handle FROM sys.conversation_endpoints WITH (NOLOCK) WHERE far_service = N'{farService}_Receiver' AND state in (N'{ConversationEndpointState.CO.ToString()}', N'{ConversationEndpointState.SI.ToString()}', N'{ConversationEndpointState.SO.ToString()}');";
        ...
        public bool IsServiceBrokerEnabled(string connectionString)
        {
            ...
            {
                sqlConnection.Open();
                string command =$"SELECT d.is_broker_enabled FROM sys.databases d WHERE name =@name;";

                var sqlCommand = new SqlCommand(command, sqlConnection);
                var scalar = sqlCommand.AddParameter("@name",database)
                                       .ExecuteScalar();
